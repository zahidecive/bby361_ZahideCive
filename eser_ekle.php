            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="formControl5" name="yayineviID">
                    ...
                    while ($yayinevi = mysqli_fetch_assoc($yayinevleriSorgu)) {
                        echo "<option value='" . $yayinevi["yayineviID"] . "'>" . $yayinevi["yayineviAdi"] . "</option>";
                    }
                    ?>
                </select>
            </div>
            <div class="mb-3">
                <label for="formControl6" class="form-label">Konu</label>
                <select class="form-control" id="formControl6" name="konuID">
                    <option selected disabled value="">Seçiniz...</option>
                    <?php
                    $konularSorgu = mysqli_query($baglanti, "SELECT * FROM konular");
                    while ($konu = mysqli_fetch_assoc($konularSorgu)) {
                        echo "<option value='" . $konu["konuID"] . "'>" . $konu["konuAdi"] . "</option>";
                    }
                    ?>
                </select>
            </div>
