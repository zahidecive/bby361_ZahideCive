...
$sorgu = mysqli_query($baglanti,"SELECT ES.*, KN.konuAdi, YE.yayineviAdi 
                                  FROM eserler ES
                                  LEFT JOIN konular KN ON ES.konuID = KN.konuID
                                  LEFT JOIN yayinevleri YE ON ES.yayineviID = YE.yayineviID
                                  WHERE eserID = $guncellenecek");
...
<main class="flex-shrink-0">
    <div class="container">
        ...
        <form method="post" action="eser_guncelle2.php">
            ...
            <div class="mb-3">
                <label for="formControl3" class="form-label">Eser URL</label>
                <input type="text" class="form-control" id="formControl3" name="eserURL" value="<?php echo $satir["eserURL"];?>">
            </div>
            <div class="mb-3">
                <label for="formControl4" class="form-label">Yayınevi</label>
                <select class="form-select" id="formControl4" name="yayineviID">
                    <option value="">Seçiniz</option>
                    <?php
                    $yayinevleriSorgu = mysqli_query($baglanti, "SELECT * FROM yayinevleri");
                    while ($yayinevi = mysqli_fetch_assoc($yayinevleriSorgu)) {
                        $selected = ($satir["yayineviID"] == $yayinevi["yayineviID"]) ? "selected" : "";
                        echo "<option value='" . $yayinevi["yayineviID"] . "' $selected>" . $yayinevi["yayineviAdi"] . "</option>";
                    }
                    ?>
                </select>
            </div>
            <div class="mb-3">
                <label for="formControl5" class="form-label">Konu</label>
                <select class="form-select" id="formControl5" name="yayineviID">
                    <option value="">Seçiniz</option>
                    <?php
                    $konularSorgu = mysqli_query($baglanti, "SELECT * FROM konular");
                    while ($konu = mysqli_fetch_assoc($konularSorgu)) {
                        $selected = ($satir["konuID"] == $konu["konuID"]) ? "selected" : "";
                        echo "<option value='" . $konu["konuID"] . "' $selected>" . $konu["konuAdi"] . "</option>";
                    }
                    ?>
                </select>
            </div>
